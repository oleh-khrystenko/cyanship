...
| Чернетка (shadcn/ui) | Production (CyanShip) | Примітка |
|---|---|---|
...
> **`types.ts`** — створювати тільки коли компонент реально приймає props. Не створювати порожні файли "про запас".
...
- **`types.ts` не створюється за замовчуванням** — landing widgets є презентаційними без зовнішніх props (дані з i18n). Файл `types.ts` додається лише коли компонент реально приймає props.
...
// messages/uk.json & messages/en.json
{
  "landing_page": {
    ...
    "footer": {
      "copyright": "© {year} CyanShip. Усі права захищені."
    }
  }
}
```

Всі widgets використовують `useTranslations('landing_page')`. next-intl v4 підтримує `useTranslations` і в server, і в client components — окремий `getTranslations` не потрібен. Widgets без interactivity (більшість landing секцій) залишаються server components. `'use client'` додається лише де є interactivity (наприклад, FAQ toggle).
...
- Звичайні `<a href="#section">` з Tailwind класами (виняток з UI Primitives Policy — anchor links на тій самій сторінці не потребують UiButton обгортки, див. `docs/conventions/ui-primitives.md`)
...
- Server component (без `'use client'`), `useTranslations` працює і в server components (next-intl v4)
- Кожен widget викликає `useTranslations('landing_page')` самостійно
...
- [ ] Компоненти: `Component.tsx` + `index.ts` (+ `types.ts` лише якщо є props)
...
1.1 i18n ключі
    ↓
2.1 HeroSection (першим — задає тон, валідує що tokens працюють)
    ↓
2.2-2.9 Решта widgets (паралельно, незалежні, одразу mobile-first)
    ↓
