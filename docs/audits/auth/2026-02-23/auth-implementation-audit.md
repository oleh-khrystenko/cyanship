...
- нестабільні 500 помилки при порожньому email;
...
E2E імпортує повний `AppModule`, що одразу ініціалізує Mongoose/Redis провайдери.
...
Rate-limit key формується з raw email, без нормалізації (`trim().toLowerCase()`), плюс логіка `GET` + `INCR` може бути race-sensitive.

### Ризик

- Обхід ліміту через `User@Mail.com`, `user@mail.com`, ` user@mail.com `.
- Непередбачувана поведінка при конкурентних запитах.

### Що виправити

1. Нормалізувати email до одного canonical виду.
2. Ліміт будувати атомарно (`INCR` first, `EXPIRE` if first hit) або Lua.
...
## F-07 (Low) — дубльований refresh на callback route

### Де знайдено

- `apps/web/src/app/[locale]/layout.tsx:43`
...
## F-08 (Low) — web build залежить від зовнішнього доступу до Google Fonts
...
## F-09 (Low) — warnings у lint (не блокують, але зашумлюють якість)
...
2. Виправити одноразовість magic-link (atomic consume).
3. Додати OAuth hardening (`state`, обов'язковий email).

## P1 (найближчий спринт)

1. Переробити e2e-середовище (mock/in-memory або test-containers).
2. Закрити rate-limit normalization + atomicity.
