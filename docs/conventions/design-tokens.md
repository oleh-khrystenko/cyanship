...
**Landing hero tokens** (маркетингова display-шкала, `@theme` у `themes.css`):

| Токен | Tailwind-клас | Призначення |
|-------|---------------|-------------|
...
Шкала **тільки для landing hero**. Решта маркетингових поверхонь лишається на `text-4xl`–`text-6xl`; робочі поверхні — на type-scale нижче.

**Utility-класи** (`@utility` у `settings.css`):

| Клас | Призначення |
|------|-------------|
| `mask-fade-y` | Розчиняє верхній і нижній край full-bleed зображення у фон сторінки замість жорсткої лінії обрізу |
