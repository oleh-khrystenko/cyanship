 ...
 *
 * Decouples `shared/api` (HTTP layer) from higher FSD layers (`stores`,
...
export type AuthEvent =
    /**
     * Emitted when the HTTP client determines the user's session is
     * irrecoverably lost (refresh failed, no fallback). Subscribers
     * should clear any in-memory user state.
     */
    | 'session-lost';
