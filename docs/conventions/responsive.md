...
Окремий, **не ширинний** адаптивний вимір: коли layout залежить від *форми* вікна, а не від його ширини. Ширинний breakpoint такого не виражає — `1024×768` і `1024×1366` мають однакову ширину, але потребують протилежних композицій.

| Варіант | Умова                        | Коли застосовувати                                                  |
| ------- | ---------------------------- | ------------------------------------------------------------------- |
| `wide:` | `@media (min-aspect-ratio: 4/5)` | Вікно ширше, ніж 4:5 — «альбомна» форма. Landing hero: широкий кадр корабля по центру + текстова колонка збоку. Нижче порогу (портрет) — вертикальна композиція: високий кадр зверху, текст під ним. |

Оголошення: `@custom-variant` у `apps/web/src/shared/styles/themes.css`.

Правила:

- Shape-варіант **не заміняє** mobile-first: базові стилі лишаються портретними, `wide:` тільки додає.
- Нові shape-варіанти вводити тільки тоді, коли ширинний breakpoint принципово не може виразити потребу. Кожен новий — рядок у цій таблиці.
- Тестові viewport-и (розділ 7) shape-варіант **не покривають повністю**: обовʼязково перевіряти обидві орієнтації на планшеті.
